...
const results = await prisma.$queryRaw<HybridSearchResult[]>`
  ...
  ${filters?.category ? Prisma.sql`AND LOWER(TRIM(category)) = ${filters.category.toLowerCase().trim()}` : Prisma.empty}
  ${filters?.district ? Prisma.sql`AND LOWER(TRIM(district)) = ${filters.district.toLowerCase().trim()}` : Prisma.empty}
...
export class RateLimiter {
  ...
  async check(key: string): Promise<{
    ...
    const now = Date.now();
    const info = this.cache.get(key) || { count: 0, resetAt: now + this.options.windowMs };
