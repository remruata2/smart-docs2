...
export class HybridSearchService {
	...
	static async search(
		...
		try {
			...
			if (!query || !query.trim()) {
				console.log(
					`[HYBRID SEARCH] Empty query, returning all records with filters`
				);
			...
			const tsvectorResults = await this.tsvectorSearch(
				query,
				undefined,
				filters
			);
			...
			if (tsvectorResults.length === 0) {
				// If category or district filter is active, return all matching records
				// instead of relying on semantic similarity which may miss records
				if (filters?.category || filters?.district) {
					console.log(
						"[HYBRID SEARCH] Step 3 - No tsvector matches but filter active, returning all filtered records"
					);
					const allRecords = await this.getAllRecords(limit * 2, filters); // Increase limit to ensure we get all
					return {
						results: allRecords,
			...
			const fallbackResults = await this.tsvectorSearch(
				query,
				undefined,
				filters
			);
    ...
    `;
		...
		console.log(
			`[HYBRID SEARCH] Getting all records with limit: ${limit}, filters:`,
			filters
		);
	...
	private static async semanticSearchOnCandidates(
		...
		try {
			const queryEmbedding = await SemanticVectorService.generateEmbedding(
				query
			);

			// Create placeholders for the IN clause
			const placeholders = candidateIds.map((_, i) => `$${i + 2}`).join(", ");
			const params = [queryEmbedding, ...candidateIds];

			// NOTE: No similarity threshold here! These candidates already passed tsvector filter.
			// Semantic search is only used for RE-RANKING, not filtering.
			const results = (await prisma.$queryRawUnsafe(
				`
        SELECT
          id,
