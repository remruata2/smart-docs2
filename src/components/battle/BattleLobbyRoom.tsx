    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col flex-1 max-w-4xl mx-auto w-full p-4">
                ...
                {/* Tab Navigation */}
                <div className="flex bg-slate-900/50 p-1 rounded-xl mb-4 border border-slate-700/50 backdrop-blur-sm">
                    <button
                ...
                {/* Tab Content */}
                <div className="bg-slate-900/80 border border-slate-700 rounded-2xl backdrop-blur-xl overflow-hidden min-h-[400px] flex flex-col mb-6">
                    {/* PLAYERS TAB */}
                    {activeTab === 'players' && (
                        <div className="p-4 flex-1">
                            <div className="flex items-center justify-between mb-4 px-1">
                                <span className="text-sm font-medium text-slate-400">
                                    Lobby Capacity: {participants.length + 1}/8
                                </span>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-1">
                                {/* Current User */}
                                <div className={cn(
                                    "bg-slate-800/50 border rounded-xl p-3 transition-all flex items-center justify-between",
                                ...
                                {participants.map((p: any) => {
                                    ...
                                    return (
                                        <div key={p.user_id} className={cn(
                                            "bg-slate-800/50 border rounded-xl p-3 transition-all flex items-center justify-between",
                                            isPReady ? "border-emerald-500/50 bg-emerald-500/5" : "border-slate-700"
                                        )}>
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p className="font-semibold text-slate-200 text-sm truncate max-w-[100px]">{p.user?.username || "Unknown"}</p>
                                ...
                                {/* Empty Slots visualization (up to 8) */}
                                {Array.from({ length: Math.max(0, 8 - (participants.length + 1)) }).map((_, i) => (
                                    <div key={`empty-${i}`} className="border border-dashed border-slate-800 bg-slate-900/30 rounded-xl p-3 flex items-center justify-center opacity-40 h-[66px]">
                                    ...
                                    messages.map((msg) => (
                                        ...
                                        >
                                            <div className={cn(
                                                ...
                                                {msg.userId !== currentUser.id && (
                                                    <p className="text-[10px] text-slate-400 mb-1 font-bold uppercase">{msg.username}</p>
                                                )}
                            ...
                            <div className="p-3 border-t border-slate-700/50 bg-slate-800/50">
                                <div className="flex gap-2">
                                    <Input
                                        ...
                                        className="bg-slate-900 border-slate-700 text-white placeholder:text-slate-500 focus:ring-indigo-500"
                                    />
                                    <Button
                                        size="icon"
                                        onClick={sendMessage}
