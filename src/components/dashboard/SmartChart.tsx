...
export function SmartChart({ config }: { config: ChartConfig }) {
	...
	if (typeof chartData === "string") {
		try {
			...
			return (
				<div className="text-red-500 p-4">Error: Invalid chart data format</div>
			);
		}
	}
	...
	if (!Array.isArray(chartData)) {
		return (
			<div className="text-red-500 p-4">Error: Chart data must be an array</div>
		);
	}
		...
		elements.forEach((el) => {
			...
			svgColorAttrs.forEach((attr) => {
				...
				if (attrValue?.includes("oklch(")) {
					el.setAttribute(
						attr,
						sanitizeColorString(attrValue, "#3b82f6") ?? ""
					);
				}
			});
		});
	...
	const exportAsPNGWithData = async () => {
		...
		try {
			...
			// Clone the chart
			const chartClone = chartContainerRef.current.cloneNode(
				true
			) as HTMLElement;
	...
	const exportAsSVG = () => {
		...
		try {
			...
			wrapper.setAttribute(
				"height",
				svgElement.getAttribute("height") || "400"
			);
		...
		try {
			...
			if (responsiveContainer) {
				...
				const rechartsWrapper =
					responsiveContainer.querySelector(".recharts-wrapper");
				if (rechartsWrapper) {
			...
			if (!svgElement) {
				...
				svgElement = Array.from(allSvgs).find((svg) => {
					...
					const hasChartContent =
						svg.querySelector('path[d*="M"], rect, circle, line') !== null;
			...
			if (!svgElement) {
				...
				alert(
					"Chart SVG not found. Please ensure the chart is fully rendered before exporting."
				);
				return;
			}
...
<body>
    <div class="chart-container">
        ...
        <div class="chart-wrapper">
            ${
							svgData ||
							'<p style="color: red;">Error: Chart SVG could not be generated</p>'
						}
        </div>
	...
	const renderChart = () => {
		switch (config.type) {
			...
			default:
				return (
					<p className="text-red-500">Unsupported chart type: {config.type}</p>
				);
		}
	};
	...
	return (
		<div
			...
			className="w-full h-96 bg-white p-4 rounded-lg border shadow-sm my-4"
		>
