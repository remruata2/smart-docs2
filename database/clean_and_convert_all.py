#!/usr/bin/env python3
"""
Comprehensive CID Cleaner and Converter
This script will clean all formatting issues including \r\n characters and convert HTML to Markdown
"""
...
class ComprehensiveCIDCleaner:
    ...
    def html_to_markdown_comprehensive(self, html_content: str) -> str:
        """
        ...
        html_content = re.sub(r'<sup[^>]*>(.*?)</sup>', r'^\1^', html_content, flags=re.DOTALL)
        html_content = re.sub(r'<sub[^>]*>(.*?)</sub>', r'~\1~', html_content, flags=re.DOTALL)
    ...
    def convert_mysql_to_postgresql_clean(self, input_file: str, output_file: str) -> bool:
        """
        Convert MySQL SQL dump to PostgreSQL format with comprehensive cleaning
        """
        try:
            ...
            def convert_insert_html(match):
                ...
                if values_start != -1:
                    ...
                    values_part = self.html_to_markdown_comprehensive(values_part)
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"-- Converted with comprehensive HTML to Markdown conversion\n")
    ...
    def process_all_files(self) -> bool:
        """Process all CID files with comprehensive cleaning"""
        ...
        for file in os.listdir(database_dir):
            if file.startswith('CID_') and file.endswith('.sql') and not file.endswith('_clean.sql'):
        ...
        for file in cid_files:
            ...
            output_file = file_path.replace('.sql', '_clean.sql')
            
            print(f"\n🔄 Processing {file}...")
            if self.convert_mysql_to_postgresql_clean(file_path, output_file):
                successful += 1
        ...
        if failed == 0:
            ...
            print("📝 All \\r\\n characters and HTML formatting have been cleaned!")
        else:
...
def main():
    """Main function"""
    cleaner = ComprehensiveCIDCleaner()
